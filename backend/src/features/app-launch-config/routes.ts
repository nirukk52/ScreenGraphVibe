...
import type { AppLaunchConfigRequest, ApiResponse, ValidationErrors } from './types.js';
...
export async function appLaunchConfigRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { id: string } }>('/app-launch-configs/:id', async (request, reply) => {
    try {
      ...
      if (!config) {
        return reply
          .code(404)
          .send(createResponse(false, undefined, 'App launch configuration not found'));
      }
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        if (!config) {
          return reply
            .code(404)
            .send(createResponse(false, undefined, 'No default configuration found'));
        }
    ...
    try {
      ...
      if (!validationResult.success) {
        ...
        return reply
          .code(400)
          .send(
            createResponse(false, undefined, 'Validation failed', JSON.stringify(validationErrors)),
          );
      }
      ...
      return reply
        .code(201)
        .send(
          createResponse(true, config, undefined, 'App launch configuration created successfully'),
        );
    } catch (error) {
  ...
  fastify.put<{ Params: { id: string }; Body: Partial<AppLaunchConfigRequest> }>(
    '/app-launch-configs/:id',
    async (request, reply) => {
      try {
        const { id } = request.params;
        ...
        return reply
          ...
          .send(
            createResponse(
              ...
              config,
              undefined,
              'App launch configuration updated successfully',
            ),
          );
      } catch (error) {
      ...
      return reply
        ...
        .send(
          createResponse(
            ...
            undefined,
            undefined,
            'App launch configuration deleted successfully',
          ),
        );
    } catch (error) {
  ...
  fastify.post<{ Params: { id: string } }>(
    '/app-launch-configs/:id/set-default',
    async (request, reply) => {
      try {
        const { id } = request.params;
        ...
        return reply
          .code(200)
          .send(
            createResponse(true, config, undefined, 'Default configuration updated successfully'),
          );
      } catch (error) {
        if (error instanceof Error && error.message.includes('not found')) {
          return reply.code(404).send(createResponse(false, undefined, error.message));
